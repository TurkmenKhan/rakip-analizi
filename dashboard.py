...
def get_stats(kategoriler: tuple = ()) -> dict:
    ...
    if kategoriler:
        ...
        s["son_kontrol"]   = q(f"SELECT MAX(s.kontrol_zamani) FROM snapshots s JOIN isps i ON i.id=s.isp_id WHERE i.kategori IN ({ph})", k) or "—"
        ...
        s["son_kontrol"]   = q("SELECT MAX(kontrol_zamani) FROM snapshots") or "—"
